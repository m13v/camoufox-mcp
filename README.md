...
| Tool | What it does |
|---|---|
| `camoufox_open` | Launch (or attach to) a named persistent session. Returns session info. |
| `camoufox_close` | Close a session (`name="*"` closes all). |
...
| `camoufox_save_storage` | Save full `storage_state` (cookies + localStorage) to JSON. |
